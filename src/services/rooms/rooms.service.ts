...
import { Room, RoomRow } from '@/models/rooms.js';
import { ExecuteValues } from 'mysql2';

export async function getRoomsByID(user_id: number): Promise<Room[]> {
...
export async function getMessagesByRoomID(
  ...
  let request = `
        ...
        WHERE room_id = ?
        `;
  let params: ExecuteValues[] = [room_id];
  ...
  const [messages] = await db.execute<RoomRow[]>(request, params);
