...
import {
  ...
  SITE_URL,
  transporter,
} from '@/modules/mail/mail.config.js';
import { Templates, TempltesProps } from '@/models/schemas/mail.js';

function getTemplate<T extends Templates>(
  template: T,
  props: TempltesProps[T],
): string {
  ...
  const compiledTemplate = Handlebars.compile<TempltesProps[T]>(templateSource);
